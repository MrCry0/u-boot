		...
		struct key_prop *prop, uint8_t *out)
{
	...
	int ret;
	fdt64_t public_exponent = fdt64_to_cpu(prop->public_exponent);
	...
	flush_dcache_range((ulong)&public_exponent,
			   (ulong)(&public_exponent) + prop->exp_len);
